...
template<class T>
inline T __fabs(const T &val)
{
	return __fabs((double)val);
}

template<class T>
inline T __log(const T &val)
{
	return log((double)val);
}

template<class T>
inline T __sqrt(const T &val)
{
...
inline float randn()
{
  ...
  return x1 * __sqrt(-2.0 * __log( w ) / w);
}
...
//      cout << endl;
      ...
      {
        //compute temp values
        float d1 = (this->ndims + 1 - __sqrt(this->ndims+1)) / 
                (this->ndims * (this->ndims+1));
        float d2 = (1 - __sqrt(this->ndims+1)) / this->ndims - d1;
...
void LshTable::computeStats(LshOptions& opt)
{
  ...
  mean /= size;
  mean2 = __sqrt((mean2/size) - (mean*mean));
